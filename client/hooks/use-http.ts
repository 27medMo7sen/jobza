...
const setTokenAction = (token: string) => ({ type: "auth/setToken", payload: token });
...
export const useHttp = () => {
	...
	React.useEffect(() => {
        ...
        console.log("i'm hereeeeeeeeeeeeeee",token)
		if (token) {
	...
	const refresh = useCallback(async (): Promise<string> => {
		...
		try {
			...
			const response: AxiosResponse<{ token: string }> = await axios.post("/auth/refresh-token", {
				token: currentToken,
			});
	...
	const post = useCallback(
		async <T, D = unknown>(url: string, data: D, config: AxiosRequestConfig = {}): Promise<T> => {
			try {
				setIsLoading(true);
				setError(null);

				const result = await makeRequest<T>(() => axios.post<T>(url, data, config));
				return result;
			} catch (err: unknown) {
	...
	const put = useCallback(
		async <T, D = unknown>(url: string, data: D, config: AxiosRequestConfig = {}): Promise<T> => {
			try {
				...
				const result = await makeRequest<T>(() => axios.put<T>(url, data, config));
				return result;
			} catch (err: unknown) {
